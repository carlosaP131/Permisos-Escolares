...
<div class="container p-5 my-5 border bg-dark text-white">
    {{-- Inicia el contenedor donde se almacenaran todos los elementos del form --}}
    ...
    <form action="{{ route('actualizar-permiso', ['idPermiso' => $permiso->id]) }}" method="POST">
        ...
        <div class="mb-3 mt-3" id="especialidadContainer" style="display: none;">
            {{-- Inicia div de la seleccion del grupo esta seccion estara oculta solo
        ...
        <div class="mb-3 mt-3">{{-- Inicia la seccion de Descripción --}}
            <textarea class="form-control" id="comment" rows="5"
                name="descripcion">{{ $permiso->descripcion }}</textarea>
        </div>{{-- Termina seccion de Descripción --}}
        <div class="d-flex justify-content-between  ">
            <button type="submit" class="btn btn-primary mb-2 mt-1">Generar Permiso</button>
            <a href="{{ route('alumno-permisos') }}" class="btn btn-danger ml-5 mt-2  ">Cancelar</a>
        </div>
